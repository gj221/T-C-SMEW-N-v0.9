...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[LAI,B,NPP,ANPP,Rg,RA,Rms,Rmr,Rmc,PHE_S,...
    dflo,AgeL,e_rel,e_relN,LAIdead,NBLeaf,...
    Sr,Slf,Sfr,Swm,Sll,...
    Rexmy,Rootl,...
    AgeDL,Bfac_day,Bfac_week,NPPm,Tsmm,NupIm,PAR_Im,NBL_Im,...
    RB,FNC,Nreserve,Preserve,Kreserve,...
    ...
    NPPI,TdpI,Bfac_weekI,NupI,NavlI,PAR_I,NBL_I,NBLeaftm1,....
    L_day,Lmax_day,Veg_Param_Dyn,cc,...
    Nreservetm1,Preservetm1,Kreservetm1,...
    Careservetm1,Mgreservetm1,Sireservetm1,...
    Nuptake,Puptake,Kuptake,...
    Cauptake,Mguptake,Siuptake,...
    FNCtm1,Se_bio,Tdp_bio,...
...
NupIm(4) = NupI(4)*(n4-1)/n4  + Cauptake/n4;
NupIm(5) = NupI(5)*(n4-1)/n4  + Mguptake/n4;
NupIm(6) = NupI(6)*(n4-1)/n4  + Siuptake/n4;
...
%%%%%%%%%%%%%%%%%%%%%
[FNC,e_relN,rNc,rPc,rKc,...
    rCac,rMgc,rSic,...
    rMc,rNcR,Navailtm1,Pavailtm1,Kavailtm1,...
    Caavailtm1,Mgavailtm1,Siavailtm1]= Nutrients_Available(Btm1,FNCtm1,Stoich,Nreservetm1,Preservetm1,Kreservetm1,...
    Careservetm1,Mgreservetm1,Sireservetm1,...
    OPT_SoilBiogeochemistry);
%%%%%%%%%%%%%%%%%%%%%%
%%% Root Exudation and transfer to Mychorriza
[Rexmy]= Root_Exudation(NPPtm1,Btm1(3),Bam,Bem,Btm1(4),rMc,rNc,rPc,rKc,rCac,rMgc,rSic,ParEx,NupIm,NavlI,EM,Tdp_bio); %%% % [gC / m^2 d]
%%%%%%%%%%%%%%%%%%%%
T_SPAN = [0 dtd];  %%%%%%%
%%%%%%%%%%%%%%%%%%%%%
sol=ode45(@VEGETATION_DYNAMIC,T_SPAN,Btm1,OPT_VD,...
    ...
    Mf,Wm,fab,fbe,Klf,ff_r,sum(Rexmy),NBLeaftm1,dflotm1,Navailtm1,Pavailtm1,Kavailtm1,...
    Caavailtm1,Mgavailtm1,Siavailtm1,...
    soCrop,TBio,GirdOpt,OPT_EnvLimitGrowth,OPT_VCA);
%%%%%%%%%%%%%%%%%%
B = deval(sol,dtd);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[LAI,NPP,Rg,RA,Rms,Rmr,Rmc,ANPP,LAIdead,Sr,Slf,Sfr,Swm,Sll,NLeaf,NLeafdead,NBLeaf,Nreserve,Preserve,Kreserve,...
    Careserve,Mgreserve,Sireserve]= VEG_DYN_RES(B,dtd,Btm1,Tam,Tsm,An,Rdark,...
...
if (aSE==2)  &&  any(ismember( jDay_cut , round(datenum(Datam(1:3))) ))
    [B,LAI,LAIdead,RB([1,7])]= Grass_Cut_Grazing(B,LAI,LAIdead,dtd,aSE,Sl,mSl,round(datenum(Datam(1:3))),jDay_cut,LAI_cut) ;
end
...
%%%%% Crop Management 
if (aSE==5)
    [B,Nreserve,Preserve,Kreserve,Careserve,Mgreserve,Sireserve,RB,LAI,LAIdead,ManI,PHE_S,dflo,AgeL,AgeDL]...
        = Crop_Planting_Harvest(B,Nreserve,Preserve,Kreserve,Careserve,Mgreserve,Sireserve,RB,dtd,LAI,LAIdead,PHE_S,dflo,AgeL,AgeDL,Datam,Mpar);
end
%%%%%
%%%%%%%%% LAI Below Minimum
LAI(LAI<LAI_min)=0;

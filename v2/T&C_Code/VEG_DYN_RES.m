...
%%% Boer 2003
function[LAI,NPP,Rg,RA,Rms,Rmr,Rmc,ANPP,LAIdead,Sr,Slf,Sfr,Swm,Sll,NLeaf,NLeafdead,NBLeaf,Nreserve,Preserve,Kreserve,...
...
TexCa = rCac*(1-ftransL*(rCac<=1))*Sll./Call + rCac*Sfr/Calf + rCac*(1-ftransR*(rCac<=1))*Sr./Calr  + Swm/Calh ;
TexMg = rMgc*(1-ftransL*(rMgc<=1))*Sll./Magl + rMgc*Sfr/Magf + rMgc*(1-ftransR*(rMgc<=1))*Sr./Magr  + Swm/Magh ;
TexSi = rSic*(1-ftransL*(rSic<=1))*Sll./Sill + rSic*Sfr/Silf + rSic*(1-ftransR*(rSic<=1))*Sr./Silr  + Swm/Silh ;
...
% Total Calcium Plant [gCa/m^2 PFT]
TCALtm1 =  Btm1(1)/Call + Btm1(2)/Cals + Btm1(3)/Calr + Btm1(4)/Calc + Btm1(5)/Calf  + Btm1(6)./Calh  ;  
% Total Magnesium Plant [gMg/m^2 PFT]
TMAGtm1 =  Btm1(1)/Magl + Btm1(2)/Mags + Btm1(3)/Magr + Btm1(4)/Magc + Btm1(5)/Magf  + Btm1(6)./Magh  ;  
% Total Silicon Plant [gSi/m^2 PFT]
TSILtm1 =  Btm1(1)/Sill + Btm1(2)/Sils + Btm1(3)/Silr + Btm1(4)/Silc + Btm1(5)/Silf  + Btm1(6)./Silh  ;  
...
TCAL =  B(1)/Call + B(2)/Cals + B(3)/Calr + B(4)/Calc + B(5)/Calf  + B(6)./Calh  ;  
% Total Magnesium Plant [gMg/m^2]
TMAG =  B(1)/Magl + B(2)/Mags + B(3)/Magr + B(4)/Magc + B(5)/Magf  + B(6)./Magh  ; 
% Total Silicon Plant [gSi/m^2]
TSIL =  B(1)/Sill + B(2)/Sils + B(3)/Silr + B(4)/Silc + B(5)/Silf  + B(6)./Silh  ; 
...
Careserve = Careservetm1 + Cauptake*dtd + (TCALtm1 - TCAL) - TexCa*dtd; %%% [gCa/m^2]
Mgreserve = Mgreservetm1 + Mguptake*dtd + (TMAGtm1 - TMAG) - TexMg*dtd; %%% [gMg/m^2]
Sireserve = Sireservetm1 + Siuptake*dtd + (TSILtm1 - TSIL) - TexSi*dtd; %%% [gSi/m^2]
